package com.project.oba.security;

import com.project.oba.config.SecurityConfig;
...
public class JwtService {
    ...
    public Authentication verifyToken(String token) {
        if (StringUtils.isNotEmpty(token) && token.startsWith(SecurityConstants.TOKEN_PREFIX)) {
            try {
                ...
                User user = userRepository.findById(Long.parseLong(subject)).get();
    ...
    public String createToken(String subject) {
        byte[] signingKey = securityConfig.getJwtSecret().getBytes(Charset.forName("UTF-8"));
        return Jwts.builder()
